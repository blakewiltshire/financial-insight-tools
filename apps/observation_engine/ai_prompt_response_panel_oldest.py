# -------------------------------------------------------------------------------------------------
# AI Prompt & Response Panel — AI Investigation Package
# -------------------------------------------------------------------------------------------------

# pylint: disable=import-error, unused-import

# -------------------------------------------------------------------------------------------------
# Docstring
# -------------------------------------------------------------------------------------------------
"""
AI Prompt & Response Panel — Observation Engine

This panel assembles exported Decision Support System (DSS) bundles into a portable
AI investigation package.

It supports:
- selecting one or more AI personas
- adding optional investigation context
- generating portable AI Persona Instructions
- generating an Investigation Prompt & Decision Support Bundle
- downloading both components for use in ChatGPT reference personas or other AI assistants
"""

# -------------------------------------------------------------------------------------------------
# Standard library
# -------------------------------------------------------------------------------------------------
import os
import sys
import re
import json
from pathlib import Path

# -------------------------------------------------------------------------------------------------
# Path Setup — Project Standard
# -------------------------------------------------------------------------------------------------
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..")))
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..")))

# -------------------------------------------------------------------------------------------------
# Third-party Libraries
# -------------------------------------------------------------------------------------------------
import streamlit as st

# -------------------------------------------------------------------------------------------------
# Core Utilities
# -------------------------------------------------------------------------------------------------
from core.helpers import get_named_paths  # pylint: disable=import-error
from insight_loader import load_snapshot_json
from ai_prompt_templates import AI_PROMPT_TEMPLATES

# -------------------------------------------------------------------------------------------------
# Resolve paths
# -------------------------------------------------------------------------------------------------
PATHS = get_named_paths(__file__)
EXPORT_FOLDER = Path(__file__).parent / "storage" / "ai_bundles" / "exports"
EXPORT_FOLDER.mkdir(parents=True, exist_ok=True)


# -------------------------------------------------------------------------------------------------
# Helper — Deduplicate persona prompts
# -------------------------------------------------------------------------------------------------
def deduplicate_prompts(prompts: list[str]) -> str:
    """
    Deduplicate selected persona prompts while preserving order.
    """
    seen = set()
    unique = []

    for prompt in prompts:
        if prompt not in seen:
            seen.add(prompt)
            unique.append(prompt)

    return "\n\n".join(unique)


# -------------------------------------------------------------------------------------------------
# Helpers — Persona Sections
# -------------------------------------------------------------------------------------------------
def _sections_for(persona_short: str) -> list[str]:
    """
    Return default response sections by persona.
    Falls back to a neutral DSS structure when no specialised persona is matched.
    """
    common = [
        "Executive overview (3–5 bullets)",
        "Signals & evidence / mechanics",
        "Interpretation & structure",
        "Risks & uncertainties",
        "What to watch",
    ]

    by_role = {
        "Quantitative Analyst": [
            "Executive overview (3–5 bullets)",
            "Signals & distributions",
            "Correlation & regime structure",
            "Uncertainty & interpretation limits",
            "What to watch",
        ],
        "Behavioural Economist": [
            "Executive overview (3–5 bullets)",
            "Key biases & signals",
            "Feedback loops & narratives",
            "Interpretation risks",
            "What to watch",
        ],
        "Portfolio Manager": [
            "Executive overview (3–5 bullets)",
            "Diversification & exposures",
            "Regime/rotation cues",
            "Constraints & trade-offs",
            "What to watch",
        ],
        "Risk Analyst": [
            "Executive overview (3–5 bullets)",
            "Stress sources & pathways",
            "Scenario structure",
            "Fragility & buffers",
            "What to watch",
        ],
    }

    return by_role.get(persona_short, common)


def _role_line_for(persona_short: str) -> str:
    """
    Return role framing for the selected AI persona.
    """
    return {
        "Quantitative Analyst": (
            "Apply statistical framing to distributions, dependencies, volatility patterns, "
            "and probabilistic interpretation."
        ),
        "Behavioural Economist": (
            "Reveal cognitive bias, framing effects, narrative reinforcement, and reflexive "
            "feedback in interpretation."
        ),
        "Portfolio Manager": (
            "Frame insights through diversification, balance, exposure coherence, constraints, "
            "and portfolio-level trade-offs."
        ),
        "Risk Analyst": (
            "Surface fragility, scenario pathways, uncertainty, risk transmission, and potential "
            "stress propagation."
        ),
        "Fundamental Analyst": (
            "Interpret statements, cash flows, valuation drivers, operating performance, and sector "
            "context for structural quality."
        ),
        "Value Investor": (
            "Focus on intrinsic value anchors, durability, quality, valuation assumptions, and "
            "margin-of-safety framing."
        ),
        "Regulatory Advisor": (
            "Map governance, disclosure, jurisdictional, compliance, and oversight structures neutrally."
        ),
        "FinTech Innovator": (
            "Link data contracts, automation, AI layers, workflow design, and digital infrastructure "
            "to system integrity."
        ),
        "Economic Systems Architect": (
            "Design and interpret modular DSS coherence, data flows, system dependencies, and "
            "governance at scale."
        ),
        "Default Persona": (
            "Provide neutral, structured decision-support framing."
        ),
    }.get(persona_short, "Provide neutral, structured decision-support framing.")


# -------------------------------------------------------------------------------------------------
# Helpers — DSS Investigation Prompting
# -------------------------------------------------------------------------------------------------
def build_investigation_method_section() -> str:
    """
    DSS investigation method included in the AI Persona Instructions.
    """
    return """INVESTIGATION METHOD
- Treat the uploaded bundle as a completed Decision Support System (DSS) investigation.
- Review every Decision Support Snapshot contained within the bundle.
- Review every Observation contained within the bundle.
- Consider the investigation as a whole before forming conclusions.
- Identify relationships, agreements, contradictions, dependencies, and recurring themes across modules.
- Do not focus on a single snapshot simply because it contains more information.
- Do not ignore snapshots because they contain less information.
- Where evidence conflicts, explain the competing interpretations rather than selecting one without justification.
- If a snapshot contributes relatively little to the investigation, explain why.
- Clearly distinguish observed evidence from interpretation.
- Base conclusions on the combined investigation rather than individual modules."""


def build_decision_support_principles_section() -> str:
    """
    FIT decision-support principles included in the AI Persona Instructions.
    """
    return """DECISION SUPPORT PRINCIPLES
- Financial Insight Tools is a decision-support framework rather than a prediction engine.
- Decision Support Snapshots represent evidence collected from different analytical modules.
- Observations represent user-recorded context, uncertainty, assumptions, or emerging relationships.
- No single snapshot should be considered authoritative in isolation.
- Confidence increases where multiple modules independently support similar conclusions.
- Treat missing evidence differently from contradictory evidence.
- Preserve uncertainty where appropriate.
- The purpose is to improve understanding, not to provide instructions, forecasts, or investment advice."""


def build_default_investigation_prompt() -> str:
    """
    Default investigation prompt embedded inside the JSON envelope.
    """
    return """Review the complete Decision Support System investigation.

Treat every Decision Support Snapshot and Observation as evidence collected during the investigation.

Review every snapshot before reaching conclusions.
Review every observation before reaching conclusions.

Identify:
- recurring themes
- supporting evidence
- contradictory evidence
- structural relationships
- uncertainty
- assumptions
- areas requiring further investigation

Do not ignore snapshots because they contain less information than others.

Only after reviewing the complete investigation should you respond using the selected persona framework."""


# -------------------------------------------------------------------------------------------------
# Persona Bootstrap
# -------------------------------------------------------------------------------------------------
def build_system_header(
    persona_label: str,
    enforce_sections: bool = False,
    footer_enabled: bool = True,
) -> str:
    """
    Build portable AI Persona Instructions.

    For ChatGPT reference personas, these instructions are already embedded in the selected GPT.
    For standard AI assistants, these instructions should be pasted into the conversation before
    the Investigation Prompt & Decision Support Bundle.
    """
    short = persona_label.replace(" by Blake Wiltshire", "").strip()

    role_line = _role_line_for(short)
    sections = _sections_for(short)
    sections_text = "\n  ".join(f"{i + 1}) {section}" for i, section in enumerate(sections))
    enforce_note = (
        "Always use these sections, even if the user asks otherwise."
        if enforce_sections
        else "Use these sections unless asked otherwise."
    )
    footer_line = f"{short} • DSS (non-advisory) • v1.0" if footer_enabled else ""

    header = f"""You are {persona_label}.
If a “{short} — Handbook.md” or Series_Index_Glossary.md reference is available in this conversation or assistant, use it as supporting context.
If not present, follow this embedded charter.

ROLE
- {role_line}

BEHAVIOUR
- Neutral, non-advisory. Concise bullets. No recommendations or forecasts.
- Treat uncertainty as part of the analysis rather than a problem to remove.
- Preserve the distinction between observed evidence, interpretation, and possible implications.

AVOID
- Investment advice, position-taking, real-time data claims, deterministic causal claims.
- Treating correlation as causation.
- Ignoring less detailed snapshots in favour of the longest or most narrative-rich section.
- Collapsing the investigation into a single headline conclusion.

OUTPUT FORMAT
- {enforce_note}
  {sections_text}

CONSTRAINTS
- Do not request live data.
- Be explicit about uncertainty and assumptions.
- If the bundle lacks fields, state what is missing and proceed with a generic framework.
- If a snapshot contributes limited context, say so briefly rather than silently ignoring it.

{build_investigation_method_section()}

{build_decision_support_principles_section()}

FOOTER
- Append: “{footer_line}”
""".strip()

    return header


def build_json_envelope_v2(
    persona_label: str,
    perspective_prompt: str,
    bundle: dict,
    footer_enabled: bool = True,
    user_defined_prompt: str = "",
) -> dict:
    """
    Build portable JSON envelope for the investigation bundle.
    """
    short = persona_label.replace(" by Blake Wiltshire", "").strip()

    meta = {
        "role_name": persona_label,
        "behaviour": "Neutral, non-advisory. Uses concise bullets. No forecasts.",
        "avoid": [
            "Investment advice or position-taking",
            "Real-time data claims",
            "Deterministic causality from correlation",
            "Ignoring snapshots because they contain less information",
        ],
        "response_sections": _sections_for(short),
        "investigation_method": [
            "Review every Decision Support Snapshot.",
            "Review every Observation.",
            "Consider relationships across modules.",
            "Identify supporting evidence, conflicting evidence, recurring themes, and structural dependencies.",
            "State uncertainty, missing information, and assumptions.",
            "Produce the persona response only after reviewing the complete investigation.",
        ],
        "footer": f"{short} • DSS (non-advisory) • v1.0" if footer_enabled else "",
    }

    envelope = {
        "persona": persona_label,
        "meta": meta,
        "prompt": perspective_prompt or build_default_investigation_prompt(),
        "user_defined_prompt": user_defined_prompt or "",
        "preferred_format": "markdown",
        "bundle": bundle,
    }

    return envelope


# -------------------------------------------------------------------------------------------------
# JSON export sanitiser — post-processing only
# -------------------------------------------------------------------------------------------------
_SANITISE_PATTERN = re.compile(
    r"(?P<prefix>[:\[,]\s*)(?P<sym>-?Infinity|NaN)(?P<suffix>(?=[,\]\}\s]))"
)


def sanitise_json_text(json_text: str) -> str:
    """
    Replace unquoted NaN / Infinity / -Infinity with null in a JSON string.

    This operates on the final serialised text only.
    It does not touch quoted occurrences such as "NaN".
    """
    return _SANITISE_PATTERN.sub(r"\g<prefix>null", json_text)


def is_valid_json(text: str) -> bool:
    """
    Quick JSON validity check.
    """
    try:
        json.loads(text)
        return True
    except Exception:
        return False


# -------------------------------------------------------------------------------------------------
# Main Renderer
# -------------------------------------------------------------------------------------------------
def render_ai_prompt_response_panel():
    st.header("AI Investigation")
    st.caption(
        "Decision Support Snapshots and observations can be assembled into a structured investigation package. "
        "Select an AI persona, add optional investigation context, and prepare a portable AI-ready export."
    )

    # Persona Dropdown and Description
    persona_keys = list(AI_PROMPT_TEMPLATES.keys())
    persona_label = st.selectbox(
        "Choose AI Persona:",
        persona_keys,
        index=persona_keys.index("Default Persona"),
    )
    persona_config = AI_PROMPT_TEMPLATES.get(
        persona_label,
        AI_PROMPT_TEMPLATES["Default Persona"],
    )

    st.caption(f"Active Persona: `{persona_label}`")

    if desc := persona_config.get("description"):
        st.info(desc)

    # Session vars
    if "persona_prompts" not in st.session_state:
        st.session_state.persona_prompts = []

    if "user_defined_prompt" not in st.session_state:
        st.session_state.user_defined_prompt = ""

    # Add or Clear persona prompts
    col1, col2 = st.columns([1, 1])

    with col1:
        if st.button("Add Selected Persona Prompt"):
            current_prompt = persona_config["prompt"]
            st.session_state.persona_prompts.append(current_prompt)

    with col2:
        if st.button("Clear All Persona Prompts"):
            st.session_state.persona_prompts = []

    # Insight Bundle Loader
    bundle_files = sorted(
        [f.name for f in EXPORT_FOLDER.glob("*.json")],
        reverse=True,
    )
    bundle_choice = st.selectbox("Select Insight Bundle", ["None"] + bundle_files)

    bundle = {}

    if bundle_choice != "None":
        bundle_path = EXPORT_FOLDER / bundle_choice

        try:
            bundle = load_snapshot_json(bundle_path)
        except Exception as exc:
            st.error(f"❌ Failed to load bundle: {exc}")
            return

    # Manual Customisation
    st.subheader("Investigation Context (Optional)")
    user_text = st.text_area(
        "Add questions, assumptions, areas of uncertainty, or additional context you would like the AI to consider alongside the investigation.",
        value=st.session_state.get("user_defined_prompt", ""),
        height=160,
        key="manual_customisation",
    )
    st.session_state["user_defined_prompt"] = user_text

    if st.button("Clear Investigation Context"):
        st.session_state.user_defined_prompt = ""
        st.rerun()

    # Final Prompt Review
    if bundle:
        st.markdown("---")
        st.markdown("### Investigation Package")

        final_prompt_text = deduplicate_prompts(st.session_state.persona_prompts)
        user_text = st.session_state.user_defined_prompt.strip()

        active_personas = [
            key
            for key, value in AI_PROMPT_TEMPLATES.items()
            if value["prompt"] in st.session_state.persona_prompts
        ]

        persona_label_combined = " + ".join(active_personas) or persona_label

        system_header = build_system_header(
            persona_label_combined,
            enforce_sections=False,
            footer_enabled=True,
        )

        envelope = build_json_envelope_v2(
            persona_label_combined,
            final_prompt_text
            if final_prompt_text
            else AI_PROMPT_TEMPLATES.get(
                persona_label,
                {},
            ).get("prompt", build_default_investigation_prompt()),
            bundle,
            footer_enabled=True,
            user_defined_prompt=user_text,
        )

        # Show persona instructions
        st.markdown("#### AI Persona Instructions")
        st.caption(
            "For standard AI assistants, paste these instructions into the conversation before the investigation bundle. "
            "If using the matching Blake Wiltshire AI Persona in ChatGPT, these instructions are already built into the persona."
        )
        st.code(system_header, language="text")

        # Sanitise exported JSON text
        raw_envelope_text = json.dumps(envelope, indent=2, ensure_ascii=False)
        export_text = sanitise_json_text(raw_envelope_text)

        # User message with fenced JSON
        st.markdown("#### Investigation Prompt & Decision Support Bundle")
        st.caption(
            "Paste or upload this bundle into the selected AI assistant. "
            "If using a standard AI assistant, provide the AI Persona Instructions first."
        )

        additional_text = ""

        if user_text:
            additional_text = (
                "**Additional investigation instructions (highest priority):**\n\n"
                f"> {user_text.strip()}\n\n"
            )

        user_body = (
            "Analyse the following Decision Support System (DSS) investigation using the selected AI persona.\n\n"
            "This investigation contains one or more Decision Support Snapshots together with any supporting observations collected during the investigation.\n\n"
            "Before producing your response:\n\n"
            "1. Review every Decision Support Snapshot.\n"
            "2. Review every Observation.\n"
            "3. Consider relationships across all modules rather than interpreting each independently.\n"
            "4. Identify supporting evidence, conflicting evidence, recurring themes, and structural dependencies.\n"
            "5. Highlight uncertainty, assumptions, missing evidence, and areas requiring additional investigation.\n"
            "6. Only after reviewing the complete investigation, produce the requested response using the selected persona.\n\n"
            "Do not ignore snapshots because they contain less information.\n"
            "Do not provide investment advice.\n"
            "Return Markdown using the required output sections.\n\n"
            + additional_text
            + "```json\n"
            + export_text
            + "\n```"
        )

        st.code(user_body, language="text")

        # Downloads
        col_dl1, col_dl2 = st.columns(2)

        with col_dl1:
            st.download_button(
                "Download AI Persona Instructions",
                data=system_header,
                file_name="ai_persona_instructions.txt",
                mime="text/plain",
                width="stretch",
            )

        with col_dl2:
            st.download_button(
                "Download Investigation Bundle",
                data=export_text,
                file_name="investigation_bundle.json",
                mime="application/json",
                width="stretch",
            )

        # API path intentionally disabled in this phase
        st.info(
            """
### Using Your AI Export

Your investigation is now ready.

#### ChatGPT Reference Personas

If you are using ChatGPT, open the selected **AI Persona by Blake Wiltshire** and provide the **Investigation Prompt & Decision Support Bundle**.

The selected AI Persona already contains the behavioural instructions, handbook, glossary, and supporting reference material.

#### Other AI Assistants

The AI Personas by Blake Wiltshire are implemented as ChatGPT reference personas.

When using another AI assistant, first provide the **AI Persona Instructions**, followed by the **Investigation Prompt & Decision Support Bundle**.

The embedded persona instructions provide a portable analytical framework. Supporting handbook, glossary, and reference material available within the ChatGPT reference implementation may not be available on other platforms.

#### Investigation Context

The exported bundle preserves the evidence, observations, Decision Support Snapshots, and investigation context developed throughout Financial Insight Tools.

Rather than beginning with a blank prompt, AI receives the structured investigation assembled during your analysis.

Interpretation remains human.

Agency remains human.

AI expands capability.
"""
        )

    # Persona Help
    with st.expander("How to Use AI Personas"):
        st.markdown(
            """
**AI Personas by Blake Wiltshire** guide interpretation using different decision lenses.

- Select a persona to apply a role-aligned analytical perspective.
- Add multiple persona prompts when you want a blended perspective.
- Use **Investigation Context** to add specific questions, assumptions, or areas of uncertainty.
- The AI Investigation Package is designed to review the complete bundle, not only the largest snapshot.

For ChatGPT reference personas, open the selected Blake Wiltshire AI Persona and provide the Investigation Prompt & Decision Support Bundle.

For other AI assistants, provide the AI Persona Instructions first, then the Investigation Prompt & Decision Support Bundle.
"""
        )

    st.markdown(
        """
---
⚠️ **Disclaimer**  
This tool does not provide investment advice. All decisions remain the responsibility of the end user.
"""
    )
