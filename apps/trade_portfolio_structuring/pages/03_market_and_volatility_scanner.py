...
from render_macro_interaction_tools_panel_trade_portfolio_structuring import render_macro_interaction_tools_panel
...
# --- Preloaded Assets (User) ---
elif data_source == "Preloaded Asset Types (User)":

    user_categories = sorted(list(preloaded_assets_user.keys())) if preloaded_assets_user else []

    if not user_categories:
        st.sidebar.caption("No user preloaded assets found.")
        st.caption("📁 User asset folder is currently empty.")
        st.stop()

    asset_category = st.sidebar.selectbox("Select Asset Category", user_categories)

    assets_in_category = preloaded_assets_user.get(asset_category) or {}
    asset_names = sorted(list(assets_in_category.keys()))

    if not asset_names:
        st.sidebar.caption("No assets found in this category.")
        st.caption("📁 This user category currently contains no assets.")
        st.stop()

    asset_sample = st.sidebar.selectbox("Select Base Asset", asset_names)

    # asset_sample will always be a valid string here (because asset_names is non-empty)
    DATA_TITLE = asset_sample
    ASSET_TYPE = asset_category
    asset_path = get_user_asset_path(asset_category, asset_sample)
    st.caption("📁 Using user-supplied asset from the custom preloaded folder.")


# --- Upload my own files ---
elif data_source == 'Upload my own files':
    uploaded_file = st.sidebar.file_uploader("Upload CSV", type="csv")
    DATA_TITLE = st.sidebar.text_input("Asset Title", help="e.g. Tesla, Gold, EUR/USD")
    ASSET_TYPE = st.sidebar.selectbox("Select Asset Type", ["Select"] + list(preloaded_assets_default.keys()))
    asset_sample = None  # not used in this path
    asset_path = None

    if uploaded_file and (DATA_TITLE.strip() == "" or ASSET_TYPE == "Select"):
        st.sidebar.warning("Provide both a title and valid asset type for the upload.")
...
if show_ai_export:
    render_ai_export_panel(
        snapshot_results=asset_snapshot_insight,
        base_asset=DATA_TITLE,
