...
from render_macro_interaction_tools_panel_trade_portfolio_structuring import (
render_macro_interaction_tools_panel
)
from macro_insight_sidebar_panel_price_action import render_macro_sidebar_tools

# -------------------------------------------------------------------------------------------------
# AI Export Tools — Price Action (Group A)
# -------------------------------------------------------------------------------------------------
from ai_export_ui_panel_price_action import render_ai_export_panel
...
# --- User Preloaded Assets ---
    ...
    asset_sample = st.sidebar.selectbox("Select Base Asset", list(preloaded_assets_user[asset_category].keys()))
    if asset_sample:
        DATA_TITLE = asset_sample
        ASSET_TYPE = asset_category
...
for tab, timeframe, data_slice, tab_key in [
    ...
    with tab:
        ...
        with tab1a:
            ...
            AgGrid(summary_df, gridOptions=gb.build(), height=500, fit_columns_on_grid_load=True,
...
# -------------------------------------------------------------------------------------------------
theme_code = "price_action"
theme_title = "Price Action & Trend Confirmation"
...
show_observation, show_ai_export, show_log = render_macro_sidebar_tools(
    ...
    selected_use_case=use_case_name
)

# -------------------------------------------------------------------------------------------------
# Derive Asset Context (Selected Asset for Trade Insight)
# -------------------------------------------------------------------------------------------------
try:
...
if show_ai_export:
    render_ai_export_panel(
        ...
        execution_readiness_label="📊 Execution Readiness Summary",
        score_explanation=f"Evaluating {DATA_TITLE} for execution readiness.",
        predisposition=predisposition,
        use_case_name=use_case_name,  # ← This must be the canonical key
        metadata_lookup=PRICE_ACTION_METADATA,
