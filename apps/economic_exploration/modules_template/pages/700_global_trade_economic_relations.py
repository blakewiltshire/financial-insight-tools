...
# -------------------------------------------------------------------------------------------------
"""
Global Trade & Economic Relations — Thematic Indicator Module
---------------------------------------------------------------

This module provides a structured decision-support interface for exploring international trade flows,
cross-border economic dependencies, and external sector balance.

It focuses on:
- Export and import values, volumes, and growth trends
- Trade balances and current account signals
- Optional indicators on bilateral trade, openness, or trade dependency
...
from indicator_map.indicator_map_700 import get_indicator_maps

from use_cases.use_case_700 import get_use_cases, render_use_case_selector

from insights.insight_700 import generate_econ_insights

from routing.routing_700 import get_indicator_input

from scoring_weights_labels.scoring_weights_labels_700_global_trade_economic_relations import (
    get_alignment_score_label,
    get_indicator_weight,
)

from visual_config.visual_config_700 import (
...
THEME = "global_trade_economic_relations"  # Theme slug (used in file lookup, visuals, AI bundles)
THEME_ID = "700"                # Theme ID
STRUCTURAL_FOLDER = "700_balance_of_payments"   # Primary dataset folder name under /data_sources/
COMPOSITE_FOLDER = ""                 # Optional second template folder
DEFAULT_USE_CASE = "Country External Balance"   # Default focus in use case selector (must match use_case key)
...
DATASET_REGISTRY = {
    "df_primary": {
        "label": "📄 Balance of Payments (Monthly)",
    ...
    "df_secondary": {
        "label": "📄 Balance of Payments (Quarterly)",
    ...
    "df_extended": {
        "label": "📄 Balance of Payments (Annual)",
        "file": f"{COUNTRY_CODE}_a_{THEME_ID}_structural.csv",
        "folder": STRUCTURAL_FOLDER,
        "frequency": "annual",
        "cleaner": clean_economic_data,
        "show_in_underlying_data": True,
...
st.caption("*Trade balances, export-import flows, and cross-border economic integration signals.*")
...
# -------------------------------------------------------------------------------------------------
df_map = {
    "trade_flows_monthly": df_primary,
    "capital_flow_quarterly": df_secondary,
    "external_position_annual": df_extended,
}
