...
# -------------------------------------------------------------------------------------------------
from universal_routing_1000 import get_indicator_input as get_indicator_input_universal
...
def get_indicator_input(indicator_name: str, df_dict: dict) -> pd.DataFrame | None:
    ...
    Logic:
    - Applies US-local routing for dollar transmission indicators
    ...
    # ---------------------------------------------------------------------------------------------
    # Dollar Strength and Global Transmission — US Local
    # ---------------------------------------------------------------------------------------------
    if indicator_name == "Dollar Strength Positioning":
        return df_dict.get("df_primary_slice")

    if indicator_name == "Trade Weighted Dollar Pressure":
        return df_dict.get("df_primary_slice")

    if indicator_name == "Cross Market Transmission":
        return df_dict.get("df_primary_slice")
