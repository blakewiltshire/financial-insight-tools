...
# -------------------------------------------------------------------------------------------------
"""
Inflation Price Dynamics — Thematic Indicator Module
--------------------------------------------------------

This module provides a structured decision-support interface for evaluating inflation trends,
price level shifts, and underlying cost drivers within an economy.

It focuses on:

- Headline inflation measures such as CPI and PPI (month-on-month, year-on-year)
- Core inflation signals excluding volatile items
- Optional price dispersion, housing costs, and energy/fuel components
...
from indicator_map.indicator_map_400 import get_indicator_maps

from use_cases.use_case_400 import get_use_cases, render_use_case_selector

from insights.insight_400 import generate_econ_insights

from routing.routing_400 import get_indicator_input

from scoring_weights_labels.scoring_weights_labels_400_inflation_price_dynamics import (
    get_alignment_score_label,
    get_indicator_weight,
)

from visual_config.visual_config_400 import (
...
THEME = "inflation_price_dynamics"  # Theme slug (used in file lookup, visuals, AI bundles)
THEME_ID = "400"                # Theme ID
STRUCTURAL_FOLDER = "400_headline_core_inflation"   # Primary dataset folder name under /data_sources/
COMPOSITE_FOLDER = ""                 # Optional (leave blank if unused)
DEFAULT_USE_CASE = "Inflation Pressure and Transmission"   # Default focus in use case selector (must match use_case key)
...
st.caption("*Price trends, cost pressures, and inflationary signals across economic cycles.*")
...
# Context message
st.caption("📌 Each 'Observation' corresponds to one data entry, based on the dataset's own frequency.")
