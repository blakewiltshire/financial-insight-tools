...
COUNTRY_CODE = "us"                 # Pulls CSV from country-specific data folders
...
STRUCTURAL_FOLDER = ""                    # Primary folder is defined per dataset entry below
COMPOSITE_FOLDER = ""                     # Optional (leave blank if unused)
DEFAULT_USE_CASE = "Currency Regime Framework"   # Default focus in use case selector
...
#     - file: CSV file name
#     - folder: The subdirectory name under /data_sources/economic_data/{country_code}/
#     - frequency: Dataset frequency used for slicing and display logic
...
#     - create_slice: Whether to create a timeframe slice
#
...
DATASET_REGISTRY = {
    "df_primary": {
        "label": "📄 Trade Weighted Currency Index",
    ...
    "df_secondary": {
        "label": "📄 Official Reserves Excluding Gold",
...
#
# For this module, df_primary is the daily broad dollar dataset.
# -------------------------------------------------------------------------------------------------
...
df_dict = {
    ...
    "df_secondary_slice": df_secondary_slice if "df_secondary_slice" in globals() else None,
    "df_tertiary_slice": df_tertiary_slice if "df_tertiary_slice" in globals() else None,
    "df_full": df_primary,
}
...
# Context message
st.caption(
    "📌 Each 'Observation' corresponds to one data entry, based on the dataset's own frequency. "
...
#
# Use semantic keys for mixed-frequency visuals.
# -------------------------------------------------------------------------------------------------
df_map = {
    "currency_index_daily": df_primary,
    "reserves_monthly": df_secondary,
    "current_account_quarterly": df_tertiary,
}
...
st.caption(
    "⚠️ One or more components may be excluded from the chart due to unavailability "
    "of time series data."
)
