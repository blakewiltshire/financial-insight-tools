...
# -------------------------------------------------------------------------------------------------
"""
Monetary Indicators Policy Effects — Thematic Indicator Module
--------------------------------------------------------

This module provides a structured decision-support interface for analysing {short_description}.

This module provides a structured decision-support interface for analysing central bank activity,
monetary aggregates, and policy-driven macro signals.

It focuses on:

- Interest rates, money supply aggregates (e.g., M1, M2), and base monetary conditions
- Central bank balance sheet signals and liquidity indicators
- Policy transmission channels and monetary environment context
...
from indicator_map.indicator_map_500 import get_indicator_maps

from use_cases.use_case_500 import get_use_cases, render_use_case_selector

from insights.insight_500 import generate_econ_insights

from routing.routing_500 import get_indicator_input

from scoring_weights_labels.scoring_weights_labels_500_monetary_indicators_policy_effects import (
    get_alignment_score_label,
    get_indicator_weight,
)

from visual_config.visual_config_500 import (
...
#         STRUCTURAL_FOLDER = "100_gdp"
#         COMPOSITE_FOLDER = "100_gdp_macro_composite"
...
THEME = "monetary_indicators_policy_effects" # Theme (used in file lookup, visuals, AI bundles)
THEME_ID = "500"  # Theme ID
STRUCTURAL_FOLDER = "500_money_supply"   # Primary dataset folder name under /data_sources/
COMPOSITE_FOLDER = "500_interest_rate"   # Optional (leave blank if unused)
DEFAULT_USE_CASE = "Money Supply and Velocity Dynamics"   # Default focus in use case selector (must match use_case key)
...
#
# • Each key becomes a variable (e.g., df_primary) available across use case logic,
...
DATASET_REGISTRY = {
    ...
    "df_secondary": {
        "label": "📄 Money Velocity (Quarterly)",
...
st.title(f"{FLAG} {COUNTRY_NAME} - {THEME.replace('_', ' ').title()}")
st.caption("*Central bank actions, liquidity metrics, and monetary policy impacts on growth.*")
...
df_dict = {
    ...
    "df_monthly_rates_slice": df_monthly_rates_slice,
    "df_weekly_rates_slice": df_weekly_rates_slice,
    "df_full": df_primary,
    "df_secondary_full": df_secondary,
    "df_extended_full": df_extended,
    "df_monthly_rates_full": df_monthly_rates,
    "df_weekly_rates_full": df_weekly_rates,
}
...
# -------------------------------------------------------------------------------------------------
# Timeframe Tab Definition Based on Observations
# -------------------------------------------------------------------------------------------------
def define_timeframe_tabs_and_mapping() -> tuple[dict, dict]:
    """
    Defines Streamlit timeframe tabs and maps each tab to a period label.
    Actual dataframe slicing is handled inside the local visual config.
    """
    ...
    tab_mapping = {
        tab_3: "Last 3 Periods",
        tab_6: "Last 6 Periods",
        tab_12: "Last 12 Periods",
        tab_24: "Last 24 Periods",
        tab_60: "Last 60 Periods",
        tab_full: "Full History"
    }

    return tabs_dict, tab_mapping

# --- Timeframe Tabs and Mapping (Frequency-Agnostic Style) ---
tabs_dict, tab_mapping = define_timeframe_tabs_and_mapping()

# Context message
st.caption("📌 Each 'Observation' corresponds to one data entry, based on the dataset's own frequency. (Quarterly, Monthly, Weekly, Daily)")
...
# -------------------------------------------------------------------------------------------------
df_map = {
    "money_supply_monthly": df_primary,
    "money_velocity_quarterly": df_secondary,
    "funding_rates_daily": df_extended,
    "policy_rates_monthly": df_monthly_rates,
    "mortgage_rates_weekly": df_weekly_rates,
}
