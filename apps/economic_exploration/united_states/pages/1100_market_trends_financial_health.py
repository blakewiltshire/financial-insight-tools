...
# -------------------------------------------------------------------------------------------------
"""
1100_market_trends_financial_health.py
--------------------------------------------------------

This module provides a structured decision-support interface for analysing {short_description}.

This module provides a structured decision-support interface for analysing capital market trends
and the underlying financial health of listed sectors or economies.

It focuses on:

- Equity index performance and valuation trends
- Market breadth, sector rotation, and capitalisation dynamics
- Optional metrics on earnings, leverage, or balance sheet quality
...
from indicator_map.indicator_map_1100 import get_indicator_maps

from use_cases.use_case_1100 import get_use_cases, render_use_case_selector

from insights.insight_1100 import generate_econ_insights

from routing.routing_1100 import get_indicator_input

from scoring_weights_labels.scoring_weights_labels_1100_market_trends_financial_health import (
    get_alignment_score_label,
    get_indicator_weight,
)

from visual_config.visual_config_1100 import (
...
THEME = "market_trends_financial_health"  # Theme slug (used in file lookup, visuals, AI bundles)
THEME_ID = "1100"                # Theme ID
STRUCTURAL_FOLDER = "1100_equity_index_template"   # Primary dataset folder name under /data_sources/
COMPOSITE_FOLDER = "1100_surprise_index_template"     # Optional
DEFAULT_USE_CASE = "Aggregate Equity Allocation"   # Default focus in use case selector (must match use_case key)
...
st.caption("*Broad financial market valuations, sector rotation signals, and asset-level momentum patterns.*")
