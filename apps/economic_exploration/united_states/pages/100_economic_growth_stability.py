...
# -------------------------------------------------------------------------------------------------
"""
Economic Growth Stability — Thematic Indicator Module
--------------------------------------------------------

This module provides a structured decision-support interface for evaluating a country’s economic
growth and macroeconomic stability.

It focuses on:
- Real GDP and Nominal GDP signals (levels and growth rates)
- Component-level GDP breakdowns (consumption, investment, government expenditure, net trade)
- Optional composite indicators and macroeconomic signal scoring (where available)

The purpose is not to forecast economic performance, but to surface structural insights and
comparative dynamics across:
- Cyclical positioning and inflection points
- Output volatility and resilience
- Policy framing and macroeconomic coordination
...
(e.g., GDP indicators from FRED or ONS)
- Country-specific files follow a standard template format (e.g., `gdp_template.csv`)
...
It provides structural framing for macroeconomic exploration and strategic alignment.
...
from indicator_map.indicator_map_100 import get_indicator_maps

from use_cases.use_case_100 import get_use_cases, render_use_case_selector

from insights.insight_100 import generate_econ_insights

from routing.routing_100 import get_indicator_input

from scoring_weights_labels.scoring_weights_labels_100_economic_growth_stability import (
    get_alignment_score_label,
    get_indicator_weight,
)

from visual_config.visual_config_100 import (
...
THEME = "economic_growth_stability"  # Theme slug (used in file lookup, visuals, AI bundles)
THEME_ID = "100"                     # Theme ID
STRUCTURAL_FOLDER = "100_gdp"        # Primary dataset folder name under /data_sources/
COMPOSITE_FOLDER = "100_gdp_macro_composite"    # Optional (leave blank if unused)
DEFAULT_USE_CASE = "Real GDP"        # Default focus in use case selector (must match use_case key)
...
#     you can register additional datasets here (e.g., df_secondary).
#     These are typically integrated once the module is fully extended.
# -------------------------------------------------------------------------------------------------

DATASET_REGISTRY = {
    "df_primary": {
        "label": "📄 GDP and Components",
    ...
    "df_secondary": {
        "label": "📄 Monthly Macro Composite Indicators",
    ...
    "df_extended": {
        "label": "📄 Weekly Economic Index (WEI)",
...
ABOUT_APP_MD = os.path.join(ROOT_PATH, "docs", f"about_{THEME}.md")   # App overview (shown in UI)
HELP_APP_MD = os.path.join(ROOT_PATH, "docs", f"help_{THEME}.md") # Help/instructions (shown in UI)
ABOUT_SUPPORT_MD = os.path.join(ROOT_PATH, "docs", "about_and_support.md") # Universal support
BRAND_LOGO_PATH = os.path.join(ROOT_PATH, "brand", "blake_logo.png")  # App logo (footer or sidebar)
...
st.caption("*Core GDP indicators and real-economy growth patterns.*")
